...
use crate::interface::{teller::teller_queue::*, user::models::*};
use crate::Servers;
...
impl QueueStruct {
        ...
        if self.queue.len() < CUSTOMER_COUNT {
            match Self::assign_users(item.clone(), self.queue.len() + 1, servers_queues) {
                ...
                Ok((teller_loc, server_queue_pos)) => {
                    // let user_pos = ;
                    if let Ok(teller_id) = teller_queue.find_teller(teller_loc - 1) {
                        // let user_pos = ;
                        let timer =
                            show_user_waiting_time(teller_id.server_id, self, server_queue_pos);
                        let user_pos: UserQueuePos = UserQueuePos {
                            national_id: item.user_query.national_id,
                            action: item.action,
                            queue_pos: self.queue.len() + 1,
    ...
    pub fn remove_item(
        ...
        if user_queue_pos < CUSTOMER_COUNT {
            let removed_user: UserQueuePos = queue.remove(user_queue_pos);
